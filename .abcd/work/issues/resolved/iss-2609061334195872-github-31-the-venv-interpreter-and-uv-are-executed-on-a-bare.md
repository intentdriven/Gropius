...
resolution: "Executables moved per-account: uv, the venv and the uv-managed CPython tree now resolve under each account's own directory via config.ExecRoot, while models stay shared. The maintainer chose per-account executables over a shared runtime with an owner check."
impact: breaking
---
...
- pursued: we expect removing the shared-execution path to end this class outright, because no account can rewrite a binary another account runs; we are wrong if a later finding shows two accounts still executing one provisioned tree
