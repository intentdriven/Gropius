...
resolution: "Eviction now keeps a stopped model server's charge in a drain tally until its process exits, and the replacement waits for that exit instead of launching on top of it."
impact: fix
---
...
- pursued: the budget is never exceeded by a victim and its replacement coexisting, because no server is launched while residentChargeLocked (live entries plus the drain tally) plus the new load is over the budget; a load admitted while a stopped process has not fired Done, or a pool whose budget stays reduced after one exits, would show it wrong.
