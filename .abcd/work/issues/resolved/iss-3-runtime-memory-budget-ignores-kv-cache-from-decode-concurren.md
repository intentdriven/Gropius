...
resolution: "The memory budget now charges a loaded model its weights, a fifth for the working set, and the attention cache its declared window costs per admitted sequence — the per-token figure read from the model's own config.json (full-attention layers, KV heads, head dimension, or the latent width) and multiplied by a safety factor of seven, with the flat 1.2x kept for a model whose configuration cannot be read and the whole budget as the ceiling on any one model."
impact: breaking
---
...
- pursued: a model is admitted only when the memory it was measured taking at the window it serves fits the budget, so two long-context models no longer co-reside on a Mac that cannot hold both — capability.LoadCostOf is the one home of the charge and the pool, the pinned-set check and the control panel all read it. What would show it wrong: a Mac that swaps or OOMs while the pool believes it is inside the budget (most likely through the budget ceiling, which deliberately charges less than the arithmetic), or a model measured holding more than seven times its configuration's per-token figure.
