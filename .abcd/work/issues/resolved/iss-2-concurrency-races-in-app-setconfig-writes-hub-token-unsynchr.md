...
resolution: "Serialised deleting a model against downloads and loads on a per-model claim under dlMu, pinned each download's Hub token, counted the wait group under the same lock as the registration, and guaranteed the download handle's done channel closes on every exit path."
impact: fix
---
...
- pursued: a Delete and a concurrent Download or request-triggered load of one model now end either as a complete download or a clean removal, never as a registered model whose directory was half unlinked; SetConfig no longer races a live download over the Hub token and a download finishes under the token it started with; a Download after Close is refused rather than leaked past dlWG; and a Download whose first Registry.Put fails releases the handle a Delete may be waiting on. Held by five tests in internal/app/concurrency_test.go under -race. What would show it wrong: a deadlock between a load and a delete (something taking dlMu and then p.mu, inverting adr-2609070004056820), a Delete that returns while the pool still holds an entry for the model it removed, or a -race finding in internal/app under repeated runs.
