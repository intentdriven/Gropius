...
| **The request log** | That each request to the API is written to the server log as method, path, status and duration, with no client address, prompt, answer or key. | A fact about Gropius, not of this server. |
