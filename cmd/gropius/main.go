...
func main() {
	...
	if cfg.ExposedToLAN() && cfg.APIKey == "" {
		...
		default:
			...
			// included, and without the keys this build ignores — so there is
			// nothing left in it to warn about.
	...
	if err := runServer(ln, paths, cfg, start.Notices, *headless, log); err != nil {
...
func runServer(ln net.Listener, paths config.Paths, cfg config.Config, notices config.Notices, headless bool, log *slog.Logger) error {
